...
var gHiddenWin = {
  get window () {
    var assClass = Components.classes["@mozilla.org/appshell/appShellService;1"];
    var ass = assClass.getService(Components.interfaces.nsIAppShellService);
    return ass.hiddenDOMWindow;
  }
}
...
function YBookmarkSyncService_delete_diff (hashes) {
  ...
  var sqliteStore = Components.classes["@yahoo.com/nsYDelLocalStore;1"].
					  getService(Components.interfaces.nsIYDelLocalStore);
  var urlhash, url;
  for (var i = 0, hashlen = hashes.length; i < hashlen; i++) {
...
function _convertHashesFromNSArrayToJSObject(nsArray) {
  ...
  var post, urlhash;
  for (var i=0, arrlen=nsArray.length; i < arrlen; i++) {
...
function _compareBookmarkHashes(localHashList, remoteHashList) {
  ...
  const NSString = new Components.Constructor("@mozilla.org/supports-string;1",
   ...
   var localMatch, str, remoteHashCount=0;
   ...
   result["remoteHashCount"] = remoteHashCount;
...
function syncSocialContacts() {
   var delreader = Components.classes[kDelContractID].
       getService(Components.interfaces.nsISocialStore);
   var cb = {
      onload: function (posts) {
              try { 
                  //yDebug.print("syncSocialContacts()=>Onload", YB_LOG_MESSAGE);			
                  var post;
                  var storeService = Components.classes["@yahoo.com/nsYDelLocalStore;1"].
                                                          getService(Components.interfaces.nsIYDelLocalStore);
                  var len = posts.length;
                  if(len) {
                      //storeService.deleteAllProviders();//Should we do this delete all the time?
                  }
                  
                  for (var i = 0; i < len; i++) {
                          post = posts.queryElementAt(i, Components.interfaces.nsIWritablePropertyBag);
                          if(post) {
                                  var provider = post.getProperty('provider');
                                  if(provider == DEL_PROVIDER_TWITTER) {
																					//update count first
                                  				var count = 0;
                                          try {
                                           count = post.getProperty('count');
                                          } catch(e){}
                                  				storeService.updateRecipientInfo(DEL_PROVIDER_TWITTER, DEL_TAG_TWITTER, count);
                                  				
                                  				//added try so that it does nothing when just a count is passed.
                                  				try {
	                                          var auth = post.getProperty('auth');
	                                          var login = post.getProperty('login');
	                                          var auto_send_public = post.getProperty('auto_send_public');
	                                          
	                                          //OAuth
	                                          try {
	                                           auth = post.getProperty('oauth');
	                                           //Set a pref to flag that oAuth is available.
	                                           Components.classes["@mozilla.org/preferences-service;1"].
	                                              getService(Components.interfaces.nsIPrefBranch).
	                                              setBoolPref("extensions.ybookmarks@yahoo.extension.twitter.OAuth.enabled", true);
	                                          } catch(e){}
	                                           storeService.updateProviderInfo(DEL_PROVIDER_TWITTER, login,
	                                               (auth == "true") ? "true" : "",
	                                               (auto_send_public == "true") ? "true" : "");
                                  				} catch(e) {}
                                  } else if (provider == DEL_PROVIDER_DELICIOUS) {
                                          var login = post.getProperty('login');
                                          var count = 0;
                                          try {
                                           count = post.getProperty('count');
                                          } catch(e){}
                                          //get username from for:username
                                          if((login.indexOf('for:') == 0) && (login.length > 4)) {
                                                  login = login.substring(4);
                                          }
                                          storeService.updateRecipientInfo(provider, login, count);
                                  } else if (provider == DEL_PROVIDER_EMAIL) {
                                          var email = post.getProperty('email');
                                          var count = 0;
                                          try {
                                           count = post.getProperty('count');
                                          } catch(e){}
                                          storeService.updateRecipientInfo(provider, email, count);
                                  }
                          }
                  }
              } catch(e) {
                  yDebug.print("syncSocialContacts()=>Onload: Exception:" + e, YB_LOG_MESSAGE); 
              }
      },		
      onerror: function (event) {
              yDebug.print("syncSocialContacts()=>Error", YB_LOG_MESSAGE);
      }
    }
    delreader.getNetworkProviders(cb);
}
/**
 * Class definition
 */
var YBookmarksSyncService = {
		...
		if(topic == "ybookmark.internalServerStatus") {
			...
			try {
				...
				try {         
					...
					disabled = pref.getBoolPref("extensions.ybookmarks@yahoo.extension.disabled");
					if(disabled) {
					    pref.setBoolPref("extensions.ybookmarks@yahoo.extension.disabled", false);
					    pref.setBoolPref("extensions.ybookmarks@yahoo.extension.disabled.confirmed", true);
					}
				} catch(e) {}				
   ...
   init: function() {
      ...
      //Sync social contacts, statuses
      syncSocialContacts();
   },
   
   cancelSync: function() {
      this._syncAllowed = false;
      gHiddenWin.window.clearTimeout(gHiddenWin.window.ybSyncTimeoutId);
   ...
   getBkmkPropertyBag: function(bookmark) {
      var propertyBag = new HashPropertyBag();
      propertyBag.setProperty("title", bookmark.name);
      propertyBag.setProperty("url", bookmark.url);
      propertyBag.setProperty("notes", bookmark.description ? bookmark.description : "");
      propertyBag.setProperty("tags", bookmark.tags ? bookmark.tags : "");
      propertyBag.setProperty("shared", bookmark.shared ? bookmark.shared : "");
      propertyBag.setProperty("shortcut", bookmark.shortcut ? bookmark.shortcut : "");
      propertyBag.setProperty("postData", bookmark.postData ? bookmark.postData : "");
      //propertyBag.setProperty("localOnly", bookmark.localOnly ? bookmark.localOnly: "");
      if(bookmark.recipients) {
          propertyBag.setProperty("recipients", bookmark.recipients);
      }
      if(bookmark.share_msg) {
          propertyBag.setProperty("share_msg", bookmark.share_msg);
      }
      return propertyBag;
   },
   ...
   processTransactions : function() {
     ...
     while (transactions.hasMoreElements() && this._syncAllowed) {
        ...
        switch (transaction.getProperty("action")) {
          ...
          case "addBookmark":
            function addBkmkCallback() {
              return {
                  bookmark: transaction.getProperty("data"),
                  ...
                  onload: function (returnValue) {
                    ...
                    sqliteStore.setTransactionState ("addBookmark", "bookmark", this.bookmark, 2);
                    yDebug.print("Added bookmark to the remote:" + url, YB_LOG_MESSAGE);
                    gHiddenWin.window.setTimeout(function() { _YBookmarksSyncHelper.updateBookmarkHash(url); }, 
                  ...
                  onerror: function (returnValue) {
                    ...
                    if(!returnValue || (returnValue.length == 0)) {
                      yDebug.print("Failed to add bookmark to the remote. No return value.", YB_LOG_MESSAGE);
                      return;
                    }  
                 ...
                 .getService(Components.interfaces.nsIObserverService)
                      ...
                      var promptService = Components.classes["@mozilla.org/embedcomp/prompt-service;1"].
		              		...
		              		promptService.alert(null, "Delicious Bookmarks", "Failed to bookmark this URL. It might have already removed.");
                    }
                    else {
                      sqliteStore.setTransactionState ("addBookmark", "bookmark", this.bookmark, 3);
                    }
            ...
            try {
                ...
                var bookmark = transaction.getProperty("data");
                if(bookmark) {
                  var propertyBag = this.getBkmkPropertyBag(YBJSON.parse(bookmark));
                  delreader.addBookmark (propertyBag, addBkmkCallback());
                } else {
                    sqliteStore.removeTransaction(transaction.getProperty("action"), transaction.getProperty("type"), transaction.getProperty("data"));
                }
            } catch(e) {
                yDebug.print("Addtransaction error:" + e, YB_LOG_MESSAGE);
            }
          break;       
          case "editBookmark":
            function editBkmkCallback() {
              return {         
                  bookmark: transaction.getProperty("data"),
                  ...
                  onload: function (returnValue) {
                    ...
                    sqliteStore.setTransactionState ("editBookmark", "bookmark", this.bookmark, 2);
                    yDebug.print("Edited Bookmark to remote :" + url, YB_LOG_MESSAGE);
                    gHiddenWin.window.setTimeout(function(url) { _YBookmarksSyncHelper.updateBookmarkHash(url); }, 
                                            DEL_HASH_REQUESET_AFTER_TRANS_WAIT,
                                            url);
                  },
                 ...
                 .getService(Components.interfaces.nsIObserverService)
                      ...
                      sqliteStore.setTransactionState ("editBookmark", "bookmark", this.bookmark, 3);
                    }
            ...
            try {
                ...
                if(bookmark) {
                  var propertyBag = this.getBkmkPropertyBag(YBJSON.parse(bookmark));
                  delreader.editBookmark (propertyBag, editBkmkCallback());
                }
            } catch(e) {     
                yDebug.print("Edit bookmark error:" + e , YB_LOG_MESSAGE);
            }
        
          break;
          case "deleteBookmark":
            function delBkmkCallback() {
              return {
                  bookmark: transaction.getProperty("data"),
                  ...
                  onload: function (returnValue) {
                    ...
                    var sqliteStore = Components.classes["@yahoo.com/nsYDelLocalStore;1"].
                                              getService(Components.interfaces.nsIYDelLocalStore);
                     sqliteStore.setTransactionState ("deleteBookmark", "bookmark", this.bookmark, 2);
                 ...
                 .getService(Components.interfaces.nsIObserverService)
                      ...
                      sqliteStore.setTransactionState ("deleteBookmark", "bookmark", this.bookmark, 3);
                    }
            ...
            var bookmark = transaction.getProperty("data");
            delreader.deleteBookmark (YBJSON.parse(bookmark).url, delBkmkCallback());
  
          break;
          case "setBundle":
          try {
            function bundleCallback() {
              return { 
                  bundle: transaction.getProperty("data"),
                  ...
                  onload: function (returnValue) {
                    ...
                    sqliteStore.setTransactionState ("setBundle", "bundle", this.bundle, 2);
                 ...
                 .getService(Components.interfaces.nsIObserverService)
                      ...
                      sqliteStore.setTransactionState ("setBundle", "bundle", this.bundle, 3);
                    }
        ...
        /*    var transEnum = transaction.enumerator;
            ...
            var sqliteStore = Components.classes["@yahoo.com/nsYDelLocalStore;1"].
					  getService(Components.interfaces.nsIYDelLocalStore);
            var bundle = transaction.getProperty("data");
            var aBundle = YBJSON.parse(bundle);
            delreader.setBundle (aBundle.name, aBundle.tags, bundleCallback());
           } catch(e) {
           }
          break;       
          case "deleteBundle":
            function delBunCallback() {
              return { 
                  bundle: transaction.getProperty("data"),
       ...
       /*              var rvEnum = rv.enumerator;
                     ...
                     //bundle holds "bundle:bundlename" need to remove "bundle:" from it
                       var bn = "bundle:";
                       if(bundle.indexOf(bn) == 0) bundle = bundle.substr(bn.length);
                     sqliteStore.setTransactionState ("deleteBundle", "bundle", this.bundle, 2);
                  ...
                  onerror: function (returnValue) {
                     ...
                     sqliteStore.setTransactionState ("deleteBundle", "bundle", this.bundle, 3);
                     yDebug.print("Failed to set bundle to the remote :" + url, YB_LOG_MESSAGE);
                  }
               }
            };       
      ...
      /*    var transEnum = transaction.enumerator;
          ...
          var bundle = transaction.getProperty("data");
          delreader.deleteBundle (YBJSON.parse(bundle).name, delBunCallback());
          break;
          default:
        }
     }
   ...
   sync: function(periodicSync) {
      ...
      if (sqliteStore.getNumberOfTransactions("all", "all", 0) > 0) {
         ...
         if (periodicSync) {
            syncTimeoutId = gHiddenWin.window.setTimeout(function(syncService) { syncService.sync(true); }, DEL_SENDING_TRANSACIIONS_WAIT, this);
            gHiddenWin.window.ybSyncTimeoutId = syncTimeoutId;
         } else {
            gHiddenWin.window.setTimeout(function(syncService) { syncService.sync(false); }, DEL_SENDING_TRANSACIIONS_WAIT, this);
         }
      ...
      if (periodicSync) {
        ...
        if (!this._isSyncing) {
          syncTimeoutId =  gHiddenWin.window.setTimeout(function(syncService) { syncService.sync(true); }, interval, this);
          gHiddenWin.window.ybSyncTimeoutId = syncTimeoutId;
        } else {
          /*          
          yDebug.print("=====> Syncing at the moment, come back 1 min later");
          syncTimeoutId =  gHiddenWin.window.setTimeout(function(syncService) { syncService.sync(true); }, DEL_IS_SYNCING_WAIT, this);
          gHiddenWin.window.ybSyncTimeoutId = syncTimeoutId;
          */
          return;
        }
   ...
   _syncFully: function(lastUpdateTime) {
      ...
      sqliteStore.clearLocalStore("fullsync");
      syncSocialContacts();
      Components.classes["@mozilla.org/observer-service;1"].
         getService(Components.interfaces.nsIObserverService).
            notifyObservers(null, "ybookmark.syncBegin", "no-update");
      gHiddenWin.window.setTimeout(this._getChunk, 0, 0, lastUpdateTime);
   },
   ...
   _syncPartially: function (dellastupdated) {
      ...
      //Get the social contacts: delicious network/email
      syncSocialContacts();
      var delreader =
         Components.classes[kDelContractID].
            getService(Components.interfaces.nsISocialStore);
      ...
      var cb = {
         onload: function(remoteHashList) {
            ...
            //calc what % of bookmarks are different
            var totalBk = lists["remoteHashCount"];
            var changeCount = downloadList.length;
            var pcntCount = totalBk * 0.1;         //10% of total bookmarks
            
            //more than 10% of bookmarks, we do a full sync instead
            if (changeCount > pcntCount && changeCount > 100) {
      ...
      var cb = {
         ...
         onload: function(posts) {
            ...
            for (var i = 1; i < posts.length; i++) {
              ...
              gHiddenWin.window.setTimeout(function(post, i) {
                 if (!YBookmarksSyncService._syncAllowed) {
                    yDebug.print("Sync cancelled.");
                    yDebug.print("Notify the observer to update all windows\n");
           ...
           if ((posts.length - 1) == DEL_CHUNK_SIZE) {
             ...
             gHiddenWin.window.setTimeout(function() { 
                if (lastUpdateTime) {
                    sqliteStore.setLastUpdateTime(lastUpdateTime); 
                } 
          ...
          gHiddenWin.window.setTimeout(function(notifyData){ 
...
var _YBookmarksSyncHelper = {
  ...
  try {
    ...
    if ( ybookmarksUtils.containsTag( tags, "firefox:rss" ) != -1 ) {
      ...
      //try to restore visit_count
      var urlVisits = bookmarksStore.getURLVisits(url);
      if(urlVisits) {
        urlVisits = urlVisits.split(" ");
        var visitCount = urlVisits[0];
        var lastVisited = urlVisits[1];
        bookmarksStore.setNumericPropertyForBookmark(url, "visit_count", visitCount);
        bookmarksStore.setNumericPropertyForBookmark(url, "last_visited", lastVisited);
      }
    }
	...
	yDebug.print("updateStoreFromPost():Exception: " + e, YB_LOG_MESSAGE);
  }	
  },
